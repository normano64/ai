...
hmmWC=function(moveInfo,readings,positions,edges,probs){
  prior=moveInfo$mem$prior
  if (is.null(prior)) {
    prior=rep(1,nrow(probs$salinity))
    prior[positions]=0
    prior=prior/sum(prior)
  } 
  trans=moveInfo$mem$trans
  if (is.null(trans)) {
    trans=matrix(rep(0,nrow(probs$salinity)*nrow(probs$salinity)),ncol=nrow(probs$salinity))
    for (i in 1:nrow(probs$salinity)) {
      options=getOptions(i,edges)
      trans[i,options]=1/length(options)
    }
    moveInfo$mem$trans=trans
  }
  prior=crossprod(prior,trans)
  for (i in 1:nrow(probs$salinity)) {
    prior[i]=prior[i]*dnorm(readings[1],probs$salinity[i,1],probs$salinity[i,2])
    prior[i]=prior[i]*dnorm(readings[1],probs$phosphate[i,1],probs$phosphate[i,2])
    prior[i]=prior[i]*dnorm(readings[1],probs$nitrogen[i,1],probs$nitrogen[i,2])
  }
  
  routes=moveInfo$mem$routes
  if (is.null(routes)) {
    routes=matrix(rep(0,nrow(probs$salinity)*nrow(probs$salinity)),ncol=nrow(probs$salinity))
    for (i in 1:nrow(probs$salinity)) {
      for (goal in 1:nrow(probs$salinity)) {
        if (i==goal) {
          routes[i,goal]=0
        } else {
          # Do route search breadth first?
          makeNode=function(a,b) {
            list(node=a,prev=c(b$prev,b$node))
          }
          boundary=c(list(node=i,prev=c()))
          while (length(boundary)>0) {
            options=getOptions(boundary[1]$node,edges)
            if (goal%in%options) {
              routes[i,goal]=makeNode(goal,boundary[1])$prev[1]
              break
            } else {
              for (opt in options) {
                boundary=c(boundary,makeNode(opt,boundary[1]))
              }
            }
            boundary=boundary[-1]
          }
    ...
    moveInfo$mem$routes=routes
  }
  moves=c(0,0)
  moves[1]=routes[positions[3],which.max(prior)]
  if (moves[1]==0) {
    moves[2]=routes[positions[3],which.max(prior[-positions[3]])]    
  } else {
    moves[2]=routes[moves[1],which.max(prior)]        
  }
  moveInfo$moves=moves
  moveInfo$mem$prior=prior
  return(moveInfo)
}
#' Run Where's Croc
#' 
